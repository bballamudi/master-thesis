...
nominal state dynamics (i.e.\ those changes not directly influenced by the 
agent). 
...
\cite{castelletti2011tree} algorithm in order to recursively build a dependency 
tree of features.
%
\begin{algorithm}[h]
...
dataset $\mathcal{D}$ of observed transitions from and environment, the values 
of a target feature $F_0^i$ in the transitions of $\mathcal{D}$, and a set 
$\mathcal{F}_{sel}^i$ of previously selected features. 
...
and the most significant feature $F^*$ according to the ranking is added to the 
