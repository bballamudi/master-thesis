...
splitting a subset of the input space according to a binary condition, in a 
...
\begin{itemize}
    ...
    \end{IEEEeqnarray}
    %
    where $S^{\mathit{All}}$ is the set of all indices in the set, 
    $S^{\mathit{T}}$ is the set of indices for which the attribute test is true,
    and $S^{\mathit{F}}$ is the set of indices for which the attribute test is
    false.
\end{itemize}
...
The procedure has two main parameters: $K$, the number of attributes randomly 
selected at each node and $n_{min}$, the minimum sample size for splitting a 
node. 
