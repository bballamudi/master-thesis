...
\begin{figure}
    ...
    \centering
    \caption[Graphical representation of the perceptron model]{Graphical representation of the perceptron model.}
    \label{f:perceptron}
\end{figure}
%
...
The processing of the input space performed by the sequence of layers that 
...
\begin{figure}
    ...
    \centering
    \caption[A fully-connected neural network with two hidden layers]{A fully-connected neural network with two hidden layers.}
    \label{f:ann}
\end{figure}
%
...
\begin{figure}
    ...
    \centering
    \caption[Visualization of SGD on a space of two parameters]{Visualization of SGD on a space of two parameters.}
    \label{f:sgd_3d}
\end{figure}
%
...
\begin{figure}
	...
	gray node represents a state, each arc leaving a state is associated to 
	an action and its corresponding reward, an the action is in turn 
	associated to the following state; note that actions may have 
	probability distributions over the outgoing arcs.}
    \label{f:mdp_graph}
\end{figure}
%
...
\]
called \textit{trajectories}, where $s_n \in S^T$, $s_0 \sim \mu$, and 
...
MDPs with discrete state and actions sets\footnote{We make this clarification for 
formality, but we do not expand the details further in this work. Refer to 
\cite{sutton1998reinforcement} for more details on the subject of non-finite MDPs.}, 
it is possible to define the concept of \textit{optimal policy} as the policy 
that maximizes the expected return collected by the agent in an episode.
...
the number of steps, and therefore it is usual to terminate the iteration according
...
Introduced by Watkins in 1992 \cite{}, and considered by Sutton and Barto 
\cite{sutton1998reinforcement} as one of the most important breakthroughs in 
